...
- (void)viewDidLoad
{
    ...
    _sizeY = (isHigherThaniPhone4_SC ? 360: 300);
    ...
    if (CGRectEqualToRect(_previewRect, CGRectZero)) {
        self.previewRect = CGRectMake(0, 0, SC_APP_SIZE.width, _sizeY +64);
    }
    [self.captureManager configureWithParentLayer:self.view previewRect:_previewRect];
...
- (void)addTopView {
    if (!_topContainerView) {
        ...
        //调整
        UIButton *closeBtn = [self buildButton:CGRectMake(16, 14, 32, 32)
                                  normalImgStr:@"close"
                               highlightImgStr:@""
                                selectedImgStr:nil
                                    ...
                                    parentView:nil];
        closeBtn.alpha = 0.6;
        [self.topContainerView addSubview:closeBtn];
    }
}
...
//    float startY = _menuView.frame.origin.y + _menuView.frame.size.height;
    CGRect bottomFrame =
    CGRectMake(0, _sizeY, SC_APP_SIZE.width, SC_APP_SIZE.height -_sizeY +20);
    
    self.bottomContainerView = [[UIView alloc] initWithFrame:bottomFrame];
    self.bottomContainerView.backgroundColor = [UIColor blackColor];
    self.bottomContainerView.alpha = 0.6;
...
- (void)addCameraMenuView {
    ...
    //拍照按钮
//    CGFloat downH = (isHigherThaniPhone4_SC ?  132: 124);
    CGFloat cameraBtnLength = 90;
    UIButton *cameraBtn = [self buildButton:CGRectMake(0, 0, cameraBtnLength, cameraBtnLength)
                               normalImgStr:@"shot.png"
                            highlightImgStr:@"shot_h.png"
                             selectedImgStr:@""
                                     action:@selector(takePictureBtnPressed:)
                                 parentView:_bottomContainerView];
    cameraBtn.center = CGPointMake(SCREEN_WIDTH /2, _bottomContainerView.frame.size.height/2 +30);
}

- (void)addMenuButtons
{
    UIButton *frontBtn = [self buildButton:CGRectMake(112 /2, 4, 54, 54)
                              normalImgStr:@"frontback"
                           highlightImgStr:@""
                            selectedImgStr:@""
    ...
    [_cameraBtnSet addObject:frontBtn];
    
    UIButton *flashBtn = [self buildButton:CGRectMake(SCREEN_WIDTH - 112/2 -54, 4, 54, 54)
                              normalImgStr:@"lightoff"
                           highlightImgStr:@""
                            selectedImgStr:@""
    ...
    [_cameraBtnSet addObject:flashBtn];
    
//    _menuView = [[UIView alloc] initWithFrame:CGRectMake(0, 360, 320, 64)];
//    _menuView.backgroundColor = [UIColor blackColor];
//    _menuView.alpha = 0.6f;
//    [self.view addSubview:_menuView];
    
    [_bottomContainerView addSubview:frontBtn];
...
//    UIImageView *imgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"touch_focus_x.png"]];
    UIImageView *imgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"tube"]];
    imgView.alpha = 0;
...
- (void)addCameraCover {
    ...
    UIView *downView = [[UIView alloc] initWithFrame:CGRectMake(0, _sizeY +64, SC_APP_SIZE.width, 0)];
...
//        [SVProgressHUD showErrorWithStatus:@"设备不支持拍照功能T_T"];
    ...
    [_captureManager takePicture:^(UIImage *stillImage) {
        //modify 4：3
        stillImage = [stillImage croppedImage:CGRectMake(0, 0, 640, 640*0.75)];
