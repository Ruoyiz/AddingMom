...
- (void)startLocate
{
    self.location.longitude = [[NSUserDefaults standardUserDefaults] longitude];
    self.location.latitude = [[NSUserDefaults standardUserDefaults] latitude];
...
- (void)getUserLocationFromGpsOnFinish:(FinishRequstBlock)aFinishBlock
{
...
- (void)getUserLocationFromIpOnFinish:(FinishRequstBlock)aFinishBlock
{
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    [manager POST:@"http://api.map.baidu.com/location/ip?ak=zCHtfQYB4GIGOl1QeiOdQgkc&coor=bd09ll" parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        //NSLog(@"ip... %@",responseObject);
        if (aFinishBlock) {
            aFinishBlock();
        }
