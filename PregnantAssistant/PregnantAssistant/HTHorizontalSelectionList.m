...
const CGFloat kHTHorizontalSelectionListLabelCellInternalPadding = 2;
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGFloat horizontalPadding = self.buttonInsets.left + self.buttonInsets.right -2;
    ...
    if ([self.dataSource respondsToSelector:@selector(selectionList:viewForItemWithIndex:)]) {
        ...
        CGFloat width = titleSize.width + horizontalPadding + kHTHorizontalSelectionListLabelCellInternalPadding +10;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.item == self.selectedButtonIndex) {
        ...
        if ([self.delegate respondsToSelector:@selector(didSelectSameButtonAtIndex:)]) {
            [self.delegate didSelectSameButtonAtIndex:indexPath.item];
        }
...
#pragma mark - 往左移动 2px width + 1px
        CGFloat left = 0;
        CGFloat width = 0;
        ...
        self.selectionIndicatorBar.frame =
        CGRectMake(cellRect.origin.x +5 + self.buttonInsets.left + kHTHorizontalSelectionListLabelCellInternalPadding/2 - self.selectionIndicatorHorizontalPadding -left,
                   self.contentView.frame.size.height - self.selectionIndicatorHeight,
                   cellRect.size.width -10 - self.buttonInsets.left - self.buttonInsets.right - kHTHorizontalSelectionListLabelCellInternalPadding + 2*self.selectionIndicatorHorizontalPadding +width,
