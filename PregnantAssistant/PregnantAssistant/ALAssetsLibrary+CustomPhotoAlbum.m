...
- (void)loadImagesAndDatesFromAlbum:(NSString *)albumName
                         completion:(void (^)(NSMutableArray *, NSMutableArray *, NSError *))completion
{
    ALAssetsLibraryGroupsEnumerationResultsBlock block = ^(ALAssetsGroup *group, BOOL *stop) {
        // Checking if library exists
        if (group == nil) {
//            completion(nil, nil, nil);
        ...
        if ([group valueForProperty:ALAssetsGroupPropertyName] != nil) {
            if ([albumName compare:[group valueForProperty:ALAssetsGroupPropertyName]] == NSOrderedSame) {
                ...
                // Execute the |completion| block
                if (completion) completion(images, dates, nil);
